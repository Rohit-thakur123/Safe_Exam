...
const normalizeCodingQuestion = (
  ...
  // Guard against null input by treating it as an empty object so callers
  // can always rely on a populated CodingQuestion.
  const q = question ?? ({} as NonNullable<CodingAssessmentProps["question"]>);
  const starterCode: Record<string, string> =
    typeof q.starterCode === "object" && q.starterCode !== null
      ? q.starterCode
      : {};
...
const codeEditor: React.FC<CodingAssessmentProps> = ({
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      const resp = await compilerAPI.post("/execute", {
        language,
        code: sourceCode,
        input: customInput,
      });
